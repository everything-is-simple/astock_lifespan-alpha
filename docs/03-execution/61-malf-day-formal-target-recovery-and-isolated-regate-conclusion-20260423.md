...
但本轮 forced audit 直接命中新 formal run `day-e687a8277f61` 后，最终 `verdict` 仍是 `部分通过`，因为 `zone_coverage = 3` 仍触发唯一软观察 flag；这里的 `zone_coverage` 只表示 `state_snapshot_sample` 的 sample coverage，不代表全量 formal target 缺区。
...
- `malf_wave_ledger.life_state.broken = 2442547`
- `malf_state_snapshot.life_state` 当前只 materialize `alive / reborn`
...
剩余问题只剩一个 sample-scope 语义软观察：

- `zone_coverage = 3`
- 该值来自 `state_snapshot_sample`，不是全量 `malf_state_snapshot`

也就是说，当前 MALF 已经不再卡在“formal target 坏了”或“rebuild 路径坏了”，而是回到 audit 表达与 sampling coverage 的尾差。
...
- 但在 `zone_coverage` 的 sample-scope 解释与文档口径收口前，`malf` 仍不放行

## 5. 下一步唯一入口

下一张卡只允许进入：

1. 把 `zone_coverage = 3` 正式解释为 `state_snapshot_sample` 的 sample coverage 观察
2. 明确四区在全量 formal snapshot 中已存在，不把该 flag 误读为 formal target 缺区
